...
use super::exec::{self, Ext};
use super::schedule::{self, Schedule};
use super::wasm::env_def::FunctionImplProvider;
...
use super::primitives::{self, ExecResult};
...
impl<'a> super::exec::Loader for WasmLoader<'a> {
    type Executable = WasmExecutable;

    fn load_init(&self, contract_key: ContractKey) -> Result<WasmExecutable, &'static str> {
        let prefab_module = load_code(contract_key, self.schedule)?;
        Ok(WasmExecutable {
            entrypoint_name: "deploy",
            prefab_module,
        })
    }
    fn load_main(&self, contract_key: ContractKey) -> Result<WasmExecutable, &'static str> {
        let prefab_module = load_code(contract_key, self.schedule)?;
        Ok(WasmExecutable {
...
impl<'a> super::exec::Vm for WasmVm<'a> {
    type Executable = WasmExecutable;
    ...
    fn execute<E: Ext>(
        ...
        let mut runtime = Runtime::new(&mut ext, input_data, &self.schedule, memory);
