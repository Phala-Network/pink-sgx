...
macro_rules! convert_args {
	() => (vec![]);
	( $( $t:ty ),* ) => ( vec![ $( { use $crate::contracts::wasm::env_def::ConvertibleToWasm; <$t>::VALUE_TYPE }, )* ] );
}
	...
	( ( $( $params: ty ),* ) -> $returns: ty ) => (
		{
			parity_wasm::elements::FunctionType::new(convert_args!($($params),*), Some({
				use $crate::contracts::wasm::env_def::ConvertibleToWasm; <$returns>::VALUE_TYPE
			}))
		}
	);
}
...
macro_rules! unmarshall_then_body {
	( $body:tt, $ctx:ident, $args_iter:ident, $( $names:ident : $params:ty ),* ) => ({
		$(
			let $names : <$params as $crate::contracts::wasm::env_def::ConvertibleToWasm>::NativeType =
				$args_iter.next()
					.and_then(|v| <$params as $crate::contracts::wasm::env_def::ConvertibleToWasm>
...
macro_rules! unmarshall_then_body_then_marshall {
	( $args_iter:ident, $ctx:ident, ( $( $names:ident : $params:ty ),* ) -> $returns:ty => $body:tt ) => ({
		let body = $crate::contracts::wasm::env_def::macros::constrain_closure::<
			<$returns as $crate::contracts::wasm::env_def::ConvertibleToWasm>::NativeType, _
		>(|| {
			unmarshall_then_body!($body, $ctx, $args_iter, $( $names : $params ),*)
		});
		let r = body()?;
		return Ok(super::sandbox::ReturnValue::Value({ use $crate::contracts::wasm::env_def::ConvertibleToWasm; r.to_typed_value() }))
	});
	( $args_iter:ident, $ctx:ident, ( $( $names:ident : $params:ty ),* ) => $body:tt ) => ({
		let body = $crate::contracts::wasm::env_def::macros::constrain_closure::<(), _>(|| {
			unmarshall_then_body!($body, $ctx, $args_iter, $( $names : $params ),*)
		});
		body()?;
...
macro_rules! define_func {
	( < E: $seal_ty:tt > $name:ident ( $ctx: ident $(, $names:ident : $params:ty)*) $(-> $returns:ty)* => $body:tt ) => {
		fn $name< E: $seal_ty >(
			$ctx: &mut $crate::contracts::wasm::Runtime<E>,
...
macro_rules! define_env {
	( $init_name:ident , < E: $seal_ty:tt > ,
		...
		impl $crate::contracts::wasm::env_def::ImportSatisfyCheck for $init_name {
		...
		impl<E: Ext> $crate::contracts::wasm::env_def::FunctionImplProvider<E> for $init_name
		{
			fn impls<F: FnMut(&[u8], $crate::contracts::wasm::env_def::HostFunc<E>)>(f: &mut F) {
